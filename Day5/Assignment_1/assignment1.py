...
def solve(cities):
    ...
    return (loop_node, all_distance)

def k_opt(loop_node,all_distance):
    N = len(loop_node)
    # 交差してる辺の数
    crossing_count = 0
    # 交差してる間続ける O(N^2 * ？？？？？？？)
        ...
        for i in range(N):
            for j in range(N):
                ...
                if a1a2_b1b2 > a1b1_a2b2:
                    ...
                    all_distance = all_distance + a1b1_a2b2 - a1a2_b1b2
        if crossing_count == 0:
            break
...
if __name__ == '__main__':
    ...
    tour, all_distance = solve(read_input(sys.argv[1]))
    tour = k_opt(tour,all_distance)
